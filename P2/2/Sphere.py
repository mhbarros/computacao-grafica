...
class Sphere:
    ...
    def __init__(self, x, y, z, radius, color=(0, 0, 0), density=50):
        self.position = (x, y, z)
        self.radius = radius
        self.color = color
        self.density = density

    def draw(self, rotation=(0, 1, 1, 1), texture=None):
        if texture is not None:
            self._draw_texturized_sphere(rotation, texture)
            return
    ...
    def _draw_texturized_sphere(self, rotation, texture):
        ...
        for i in range(0, self.density):
            glBegin(GL_TRIANGLE_STRIP)
            for j in range(0, self.density):
                glTexCoord2f((self.density - 1 - j) / (self.density - 1),
                             (self.density - 1 - i) / (self.density - 1) * -1)
                glVertex3fv(self._sphere(i, j))
                glTexCoord2f((self.density - 1 - j) / (self.density - 1),
                             (self.density - 1 - i) / (self.density - 1) * -1)
                glVertex3fv(self._sphere(i - 1, j))
